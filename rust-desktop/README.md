...
版本 **0.4.4**。独立项目目录，原项目保留作 Electron 对照基线。
...
当前发布仓库：[nalapawo654-gif/em-use](https://github.com/nalapawo654-gif/em-use)。0.4.4 新增 11 只桌宠的咚咚消息提醒、独立消息账户、自动检查版本及专属更新提示，并更新 11 只桌宠说明页。0.4.3 纳入五只新桌宠、角色动画与装扮，并将所有桌宠末档形态统一为剩余 ≤15% 触发。0.4.2 首次将咚咚自动登录、手动账户切换与启动默认选择纳入正式版本；0.4.1 安装包不含这套功能。

```sh
npm run version:set -- 0.4.4
```

该命令同步 `package.json`、`package-lock.json`、`Cargo.toml`、`Cargo.lock`、`tauri.conf.json`。CI 检查这些版本一致，稳定发布的标签必须是对应的 `v0.4.4`。提交版本变更后推送标签，即可触发完整发布。
...
└── em-use/
    ...
    ├── stable/latest.json
    └── releases/0.4.4/
        ├── EM-Use-0.4.4-windows-x86_64.exe
        ├── EM-Use-0.4.4-windows-x86_64.exe.sig
        ├── EM-Use-0.4.4-darwin-aarch64.dmg
        ├── EM-Use-0.4.4-darwin-aarch64.app.tar.gz
        ├── EM-Use-0.4.4-darwin-aarch64.app.tar.gz.sig
        ├── EM-Use-0.4.4-darwin-x86_64.dmg
        ├── EM-Use-0.4.4-darwin-x86_64.app.tar.gz
        ├── EM-Use-0.4.4-darwin-x86_64.app.tar.gz.sig
        ├── SHA256SUMS.txt
        └── version.json
```
