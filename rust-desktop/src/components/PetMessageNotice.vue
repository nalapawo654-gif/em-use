...
function visibilityChanged(){if(!document.hidden&&state.messages&&!opened.value&&!props.blocked){const arrival=claimArrival(state.messages,paused.value);if(arrival){bubble.value=arrival.key;remaining=5000;last=Date.now()}}}
...
async function open(){
  ...
  if(api.openMessagePanel){opened.value=true;try{await api.openMessagePanel()}catch{opened.value=false;error.value='请从设置查看咚咚消息。'}return}
  opened.value=true
}
watch(()=>[state.messages?.epoch,state.messages?.revision,props.blocked,state.messages?.status,opened.value],()=>{
  if(props.blocked){opened.value=false;return}
  if(state.messages?.status!=='ready'){bubble.value=undefined;return}
  const arrival=claimArrival(state.messages,opened.value||document.hidden||paused.value)
...
onMounted(()=>{document.addEventListener('visibilitychange',visibilityChanged);window.addEventListener('message-panel-closed',panelClosed);parent=host.value?.parentElement??null;document.addEventListener('keydown',escape,true);timer=setInterval(()=>{const now=Date.now(),delta=now-last;last=now;if(bubble.value&&!hovering.value&&!document.hidden&&!props.blocked&&!host.value?.querySelector(':focus-visible')){remaining-=Math.min(delta,500);if(remaining<=0)bubble.value=undefined}},100)})
onUnmounted(()=>{document.removeEventListener('visibilitychange',visibilityChanged);window.removeEventListener('message-panel-closed',panelClosed);clearInterval(timer);release();document.removeEventListener('keydown',escape,true);emit('openChange',false);emit('bubbleChange',false)})
</script>
<template>
  <div ref="host" v-show="visible && !blocked" class="pet-message-notice" :class="[`message-${state.settings.scene}`, `message-motion-${persona.motion}`,{ 'has-message-bubble':!!current, 'message-open':opened, 'message-paused':paused, 'message-gentle':state.settings.reducedMotion }]" :style="{ '--msg-color':persona.color }" data-pet-gesture @pointerdown.stop @click.stop @contextmenu.stop.prevent @wheel.stop @mouseenter="hovering=true" @mouseleave="hovering=false">
    <template v-if="!opened || isDesktop">
      <button ref="trigger" class="message-launcher" :aria-label="`${persona.object}，${count} 条新提醒，查看咚咚消息`" :aria-expanded="opened" @click="open"><MessageParcel :scene="state.settings.scene"/><span v-if="count&&!paused" class="message-count">{{ count>99?'99+':count }}</span><PhBellSlash v-if="paused" class="message-muted-badge"/><span class="message-launcher-tooltip">{{paused?'提醒已暂停':'查看咚咚消息'}}</span></button>
      <div v-if="current && !paused" class="message-bubble"><button class="message-bubble-content" @click="open"><b>咚咚 · {{ mutedPreview ? '新消息' : current.sender }}<span v-if="current.mentioned&&!mutedPreview" class="message-mention">@我</span></b><span>{{mutedPreview?'你收到了一条新消息':current.body}}</span></button><button class="message-bubble-close" aria-label="收起消息气泡" @click="bubble=undefined"><PhX/></button></div>
    </template>
