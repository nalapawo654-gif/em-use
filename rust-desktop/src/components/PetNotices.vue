...
const props=withDefaults(defineProps<{blocked?:boolean;messageBlocked?:boolean;messageDeferred?:boolean}>(),{messageBlocked:undefined})
...
<template>
  ...
  <PetMessageNotice :blocked="(props.messageBlocked ?? props.blocked)||update" :deferred="props.messageDeferred" @open-change="message=$event" @bubble-change="bubble=$event"><template v-if="$slots.parcel" #parcel><slot name="parcel"/></template></PetMessageNotice>
</template>
