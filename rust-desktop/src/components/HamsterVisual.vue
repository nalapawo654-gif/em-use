...
const props = withDefaults(defineProps<{ message?:boolean; level: HamsterLevel; skin?: HamsterSkin; action?: HamsterAction; reducedMotion?: boolean; active?: boolean }>(), { action: 'idle', active: true, skin: 'classic' })
...
function draw(time: number) { if (frames && canvas.value) { const ctx = canvas.value.getContext('2d'); if (ctx) canvas.value.dataset.runFrame=String(renderHamster(ctx, frames, props.level, props.action, time, animated(), props.skin, props.message&&mail?.active.value?mail.paint:undefined)) } }
...
watch(()=>[props.level,props.action,props.reducedMotion,props.active,mail?.active.value],update)
