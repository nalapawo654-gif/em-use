...
async(page)=>{
 ...
 const pets=[];for(const name of await page.locator('[data-pet]').allTextContents()){await page.getByRole('button',{name,exact:true}).click();await page.locator('#pet-image').evaluate(i=>i.decode());pets.push({name,title:await page.locator('#pet-title').textContent(),pressed:await page.getByRole('button',{name,exact:true}).getAttribute('aria-pressed')});}
 ...
 if(pets.length!==11||errors.length||widths.some(w=>w.scrollWidth>w.width||w.broken.length)||pets.some(p=>p.title!==p.name||p.pressed!=='true')||faqOpen!==1)throw Error(JSON.stringify({errors,widths,pets,faqOpen}));return{errors,widths,pets,downloads,faqOpen};
}
