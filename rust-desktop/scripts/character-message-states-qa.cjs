...
async page => {
 const scenes=['feidudu','cultivation','beaver','hamster','dinosaur','skadi'], results=[], errors=[];
 ...
 for(const scene of scenes){
  ...
  for(const gentle of [true,false])for(const percent of [100,70,50,20,15,0,null]){
   ...
   for(const action of config.actions){
    ...
    await page.clock.runFor(500);await check(scene,`${percent}/${action}/${gentle}/perform`);
    if(percent===15&&!gentle){await page.locator(`.${scene}-widget`).screenshot({path:`rust-desktop/output/playwright/character-mail-${scene}-${action}.png`});}
   }
  }
  ...
  for(const width of [440,300,190,180]){
   ...
   for(const form of (scene==='skadi'?['chibi','adult']:['default']))for(const skin of config.skins)for(const percent of [100,70,50,20,15,0,null]){
    ...
    await page.clock.runFor(150);await page.waitForFunction(()=>[...document.querySelectorAll('[data-loaded]')].every(e=>e.getAttribute('data-loaded')==='true'));if(scene==='skadi')await page.locator('.skadi-figure[data-ready=true]').waitFor();await page.locator('.message-launcher').hover();await page.clock.runFor(250);await check(scene,`${width}/${form}/${skin}/${percent}`);
    if(width===440&&percent===15){await page.mouse.move(740,430);await page.locator(`.${scene}-widget`).screenshot({path:`rust-desktop/output/playwright/character-mail-${scene}-${form}-${skin}.png`});}
   }
  }
 }
