...
for(const ext of platform.startsWith('windows')?['.exe']:['.dmg']){
  const matches=files.filter(f=>f.endsWith(ext));if(matches.length!==1){throw new Error(`Expected one ${ext} for ${platform}, got ${matches.length}`)}
  const source=matches[0],name=`EM-Use-${version}-${platform}${ext}`;copyFileSync(source,join(dest,name))
  manifest.installers.push(name)
  if(ext==='.exe'){
