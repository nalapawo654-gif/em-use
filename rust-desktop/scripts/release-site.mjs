...
export function buildSite(input,output,version,base='http://172.27.12.77:5500/') {
  ...
  const records=platforms.map(platform=>{
    ...
    if(data.version!==version||data.platform!==platform||!Array.isArray(data.installers)||data.installers.length!==1)throw new Error(`Incomplete release: ${platform}`)
    if(!data.installers[0].endsWith(windows?'.exe':'.dmg'))throw new Error('Wrong installer package type')
    if(windows&&(!data.update?.signature?.trim()||data.update.file!==data.installers[0]))throw new Error(`Incomplete Windows updater: ${platform}`)
    if(!windows&&data.update!=null)throw new Error('macOS releases must use manual DMG downloads')
    const names=[...new Set([...data.installers,...(windows?[data.update.file,`${data.update.file}.sig`]:[])])]
    ...
    if(windows&&readFileSync(join(dir,`${data.update.file}.sig`),'utf8').trim()!==data.update.signature)throw new Error('Signature sidecar mismatch')
  ...
  for(const {dir,data,names}of records){for(const name of names){const bytes=readFileSync(join(dir,name));copyFileSync(join(dir,name),join(release,name));sums.push(`${createHash('sha256').update(bytes).digest('hex')}  ${name}`)}
    if(data.update)latest.platforms[data.platform]={signature:data.update.signature,url:new URL(`em-use/releases/${version}/${encodeURIComponent(data.update.file)}`,baseURL).href}
