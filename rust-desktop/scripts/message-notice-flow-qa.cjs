async page=>{
 const base='http://127.0.0.1:5188';await page.setViewportSize({width:440,height:440});
 await page.goto(`${base}/tests/fixtures/desktop.html?scene=hamster&messages=1&random=off`);await page.locator('.message-bubble').waitFor();
 await page.locator('.message-bubble').hover();await page.waitForTimeout(5500);if(!await page.locator('.message-bubble').isVisible())throw Error('Hover did not pause');
 await page.mouse.move(439,439);await page.waitForFunction(()=>!document.querySelector('.message-bubble'),{},{timeout:7500});
 ...
 await page.goto(`${base}/tests/fixtures/desktop.html?scene=hamster&messages=1&update=available&fresh=1&motion=off&random=off`);await page.locator('.message-bubble').waitFor();
 ...
 await page.getByRole('button',{name:'收起消息气泡',exact:true}).click();await page.locator('.update-letter-button').waitFor();await page.locator('.update-letter-button').click();
 ...
 if(await page.locator('.message-bubble').isVisible())throw Error('Gameplay interrupted');await page.keyboard.press('Escape');await page.locator('.message-bubble').waitFor();
 if(!await page.locator('.message-bubble').innerText().then(t=>t.includes('待玩法结束')))throw Error('Deferred message lost');
