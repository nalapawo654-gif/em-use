...
async page => {
 ...
 for(const scene of scenes)for(const width of [440,300,190,180])for(const theme of ['day','night']){
  ...
  if(theme==='day')await page.screenshot({path:`rust-desktop/output/playwright/message-illustrated-${scene}-${width}.png`,animations:'disabled'});
  ...
  const overlaps=await page.evaluate(()=>{
   ...
   const selectors='[class$="-speech"],[class$="-hud"],[class*="-resize"],.quota-display,.quota-overlay,.buddy-sign,.beaver-sign,.hamster-quota,.cultivation-quota,.battery-quota,[class$="-quota-wrap"],[class*="-header"] button,[class*="-tools"] button,.hover-tools button';
