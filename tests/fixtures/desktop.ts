...
const state: AppState = { status: 'ready', quota: { limit: 300, used: 96, remaining: 204, percent: 68, exceeded: false, estimatedAt: '2026-09-09T18:00:00', serverAt: '2026-09-09T18:00:00', receivedAt: Date.now(), day: '2026-09-09' }, message: '仅用于组件测试 · 演示额度', syncing: false, persistentLogin: false, loginOpen: false, settings: { ...DEFAULT_SETTINGS, size, theme: 'day', outfit: 'sailor' }, version: '0.2.0-visual-test' }
const listeners = new Set<(s: AppState) => void>()
const publish = () => listeners.forEach(fn => fn(structuredClone(state)))
window.emUse = {
  async getState() { return structuredClone(state) },
  async settings(patch) { Object.assign(state.settings, patch); publish() },
