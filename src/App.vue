...
import { PhGearSix, PhMinus, PhArrowsClockwise, PhFish, PhDrop, PhCamera, PhSun, PhMoon, PhArrowRight, PhCheckCircle, PhCloudSlash, PhArrowSquareOut, PhPlant, PhSparkle, PhInfo, PhHeart, PhShieldCheck } from '@phosphor-icons/vue'
...
const showSettings = ref(view === 'settings'), details = ref(false), toast = ref(''), feedingCount = ref(0)
...
function interact(kind: string) {
  if (kind === 'feed') { feedingCount.value++; notify('开饭啦！喂食不会消耗 API 额度') }
  if (kind === 'clean') notify('亮晶晶，又是清清爽爽的一天')
...
<template>
  <main :class="['app', { native: isDesktop, night, 'settings-view': view === 'settings', 'reduced-motion': state.settings.reducedMotion }]">
    ...
    <template v-else>
      ...
      <div :class="['experience', { 'is-native': isDesktop }, isDesktop ? state.settings.size : 'standard']">
        <section class="widget" :class="{ 'has-details': details }" aria-label="额度小鱼缸">
          ...
          <div class="widget-body">
            ...
            <div class="aquarium-wrap">
              <Aquarium ref="aquarium" :percent="percent" :night="night" :reduced-motion="state.settings.reducedMotion" :compact="isDesktop && state.settings.size !== 'standard'" :muted="['stale', 'expired', 'resetting'].includes(state.status)" @interact="interact">
                <div v-if="usable && state.quota" class="inside-amount">¥{{ money(state.quota.remaining) }} <span>/ ¥{{ money(state.quota.limit) }}</span></div>
              </Aquarium>
            </div>
            <div class="interaction-tools"><button title="喂食" aria-label="喂食" @click="aquarium?.feed()"><PhFish weight="duotone"/><span>喂食</span></button><button title="清洁" aria-label="清洁" @click="aquarium?.clean()"><PhDrop weight="duotone"/><span>清洁</span></button><button title="换装" aria-label="换装" @click="openSettings"><PhPlant weight="duotone"/><span>换装</span></button><button title="截图" aria-label="截图" @click="screenshot"><PhCamera weight="duotone"/><span>截图</span></button></div>
        ...
        <aside v-if="!isDesktop" class="companion-panel">
          ...
          <div class="preview-bottom"><button @click="api.settings({ theme: night ? 'day' : 'night' })"><PhSun v-if="night"/><PhMoon v-else/>{{ night ? '切换白天' : '看看夜晚' }}</button><span>试试喂食，或者点点小鱼<PhArrowRight/></span></div>
        </aside>
      </div>
