...
export function useWindowGestures() {
  ...
  function down(event: PointerEvent, mode: WindowGesture = 'move') {
    ...
    if (mode === 'move' && (target.closest('.is-wiping, [data-pet-gesture]') || target.closest('button:not(.fish-hit):not(.scene-hit), input, a, .play-popover, .detail-popover, .play-hud, .buddy-panel, .buddy-hud, .beaver-panel, .beaver-hud'))) return
