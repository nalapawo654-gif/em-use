...
<template>
  <section class="settings-panel">
    <header class="settings-header"><div><span class="eyebrow">MAKE IT YOURS</span><h1>你的小鱼缸</h1><p>把一点可爱，留在桌面上。</p></div><button class="icon-button close-settings" aria-label="关闭设置" @click="emit('close')"><PhX/></button></header>
    <div class="settings-layout">
      ...
      <div class="settings-content">
        <template v-if="tab === 'appearance'">
          ...
          <label class="size-slider"><span>自由缩放 <b>{{ state.settings.windowWidth }} px</b></span><input aria-label="鱼缸大小" type="range" min="180" max="800" step="10" :value="state.settings.windowWidth" @input="set('windowWidth', Number(($event.target as HTMLInputElement).value))"/><small>拖鱼缸四角，或按住 Ctrl / ⌘ 滚动鼠标。</small></label>
